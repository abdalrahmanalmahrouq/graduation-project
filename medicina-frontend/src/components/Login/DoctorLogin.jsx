...
export default function DoctorLogin() {
    ...
    if(localStorage.getItem('token')){
      return <Navigate to='/doctor/account' />
    }
              ...
              .then((response)=>{
                ...
                .then((profileResponse) => {
                  ...
                  if (profileResponse.data.role !== 'doctor') {
                    setMessage(`يجب تسجيل الدخول من صفحة ${profileResponse.data.role === 'patient' ? 'المريض' : profileResponse.data.role === 'clinic' ? 'العيادة' : profileResponse.data.role === 'lab' ? 'المختبر' : profileResponse.data.role}`);
                    setLoading(false);
                    return;
                  }
    ...
    <AuthLayout title="تسجيل الطبيب">
      <form dir="rtl" onSubmit={formSubmit}>
              ...
              <div className="pb-3 mt-1">
                مستخدم جديد؟ <Link to="/register/doctor" className="links-buttons-underline">إنشاء حساب</Link>
              </div>
            </form>
    </AuthLayout>
  );
