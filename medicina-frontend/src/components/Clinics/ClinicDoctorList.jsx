  ...
  return (
    <>
      <div className="doctor-card">
        <button className="doctor-delete-btn" onClick={handleDeleteClick} title="حذف الطبيب">
        ...
        <div className="doctor-image-container">
          <div className="relative">
            ...
            />
            <div className="doctor-status"><div className="doctor-status-dot"></div></div>
          </div>
        </div>
        <h3 className="add-doctor-name">{doctor.name}</h3>
        <p className="doctor-specialty">{doctor.specialty || doctor.clinic}</p>
        <button onClick={handleManage} className="doctor-manage-btn">إدارة الطبيب</button>
      </div>
        ...
        <div className="delete-modal-overlay">
          <div className="delete-modal">
            ...
            <div className="delete-modal-body">
              <p className="delete-modal-message">هل أنت متأكد من أنك تريد حذف الطبيب <strong>{doctor.name}</strong>؟</p>
            </div>
            <div className="delete-modal-actions">
              <button onClick={() => setShowDeleteModal(false)} className="delete-modal-cancel-btn" disabled={isDeleting}>إلغاء</button>
              <button onClick={handleConfirmDelete} className="delete-modal-confirm-btn" disabled={isDeleting}>
  ...
  const handleNextStep = async () => {
    ...
    try {
      ...
      if (err.response && err.response.status === 404) {
        setError('لم يتم العثور على طبيب بهذا الرقم. يرجى التحقق من الـ ID.');
      } else {
      ...
      <div className="modern-modal-content">
        ...
        <div className="modern-modal-header">
          <h2 className="modern-modal-title">
            {showRestorePrompt ? 'استعادة طبيب' : (step === 1 ? 'إضافة طبيب - الخطوة 1' : 'تحديد الجدول - الخطوة 2')}
          </h2>
        ...
        <div className="modern-modal-body">
          ...
          {showRestorePrompt ? (
            ...
            <div className="text-center py-6 animate-fadeIn">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">الطبيب موجود سابقاً</h3>
              <p className="text-gray-600 mb-4">
                تم العثور على <strong>{foundDoctorName}</strong> في سجلات العيادة المحذوفة.
                <br />
            ...
            // --- STEP 1: DOCTOR ID ---
            <div className="py-8">
              <label className="block text-gray-700 font-bold mb-2">رقم تعريف الطبيب (ID)</label>
              <input
                type="text"
                value={doctorId}
              ...
              <p className="text-sm text-gray-500 mt-2">
                سيتم البحث عن الطبيب في النظام وإضافته إلى عيادتك.
              </p>
            </div>
          ) : (
            // --- STEP 2: SCHEDULE ---
            <div className="schedule-container">
              <p className="text-sm text-gray-600 mb-2">
                قم بتفعيل الأيام التي يعمل بها الطبيب <span className="font-bold text-brand">{foundDoctorName}</span> وحدد ساعات العمل والاستراحة.
              </p>
        ...
        <div className="modern-modal-footer">
          <button onClick={onClose} className="btn-secondary" disabled={isLoading}>إلغاء</button>

          {showRestorePrompt ? (
            <button onClick={handleRestore} className="btn-primary bg-amber-500 hover:bg-amber-600" disabled={isLoading}>
              {isLoading ? 'جاري الاستعادة...' : 'نعم، استعادة الطبيب'}
            </button>
          ) : step === 1 ? (
            <button onClick={handleNextStep} className="btn-primary" disabled={!doctorId.trim() || isLoading}>
