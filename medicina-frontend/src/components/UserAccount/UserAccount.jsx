...
const UserAccount = ({ token }) => {
	...
	const loadInsuranceOptions = async () => {
		...
		try {
			...
			const rawOptions = Array.isArray(response.data?.data)
				? response.data.data
				: [];
			...
			setInsuranceOptions([]);
			setInsuranceFetchError(
				"Failed to load insurance companies. Please try again later.",
			);
		} finally {
	...
	const validateForm = (data, role) => {
		...
		if (data.phone_number && data.phone_number.trim()) {
			...
			if (!phoneRegex.test(data.phone_number.trim())) {
				errors.push(
					"Phone number must contain only numbers, spaces, hyphens, parentheses, and optional + prefix.",
				);
			}
		}
		...
		if (data.specialty && data.specialty.trim().length > 255) {
			errors.push("Specialty cannot exceed 255 characters.");
		}
	...
	const confirmDeleteAccount = () => {
		if (deleteConfirmation !== "حذف") {
			setMessage({
				type: "error",
				text: 'يرجى كتابة "حذف" بالضبط للتأكيد',
			});
			return;
		}
		...
		axios
			...
			.catch((err) => {
				...
				if (err.response?.status === 401) {
					...
					setMessage({
						type: "error",
						text: "ليس لديك صلاحية لحذف هذا الحساب",
					});
				} else if (err.response?.status >= 500) {
					setMessage({
						type: "error",
						text: "حدث خطأ في الخادم. يرجى المحاولة مرة أخرى لاحقاً",
					});
				} else {
					setMessage({
						type: "error",
						text: "حدث خطأ أثناء حذف الحساب. يرجى المحاولة مرة أخرى",
					});
				}
			});
	};
		...
		return (
			<div className="user-account-error">
				<div className="error-container">
					...
					<p>
						يرجى المحاولة مرة أخرى أو الاتصال بالدعم إذا كان المشكلة
						مستمرة.
					</p>
				</div>
			</div>
	...
	const { user_id, full_name, phone_number, date_of_birth, address } =
		user.profile;
	...
	const getRoleColor = (role) => {
		switch (role) {
			case "patient":
				return "var(--contrast-color)"; // Green
			case "doctor":
				return "var(--contrast-color)"; // Blue
			case "clinic":
				return "var(--contrast-color)"; // Purple
			case "lab":
				return "var(--contrast-color)"; // Orange or other unique color
			default:
				return "#6b7280"; // Gray
		}
	};
	...
	const renderInputField = (
		...
		<div className="info-field modern-field">
			<div className="field-icon">
				<i
					className={`fa-solid ${getFieldIcon(field)} medicina-theme-icon`}
				></i>
			</div>
	...
	const patientForm = (
		<div className="user-info-grid">
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-id-card-clip medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-hospital-user medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
					<label className="field-label">Role</label>
					<span
						className="field-value role-badge non-editable"
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-envelope medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-building medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
					...
					>
						...
						{!hasLoadedInsurances && user.profile?.insurances && (
							<option
								value={
									user.profile.insurances.insurance_id || ""
								}
							>
	...
	const doctorForm = (
		<div className="user-info-grid">
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-user-doctor medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
					<label className="field-label">Role</label>
			...
			<div className="info-field modern-field">
				...
				<div className="field-content">
					...
					<span className="field-value non-editable">
						{user.profile.consultation_duration}{" "}
					</span>
				</div>
			</div>
	...
	const clinicForm = (
		<div className="user-info-grid">
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-hospital medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
					<label className="field-label">Role</label>
	...
	const labForm = (
		<div className="user-info-grid">
			...
			<div className="info-field modern-field">
				<div className="field-icon">
					<i
						className={`fa-solid fa-flask-vial medicina-theme-icon`}
					></i>
				</div>
				<div className="field-content">
					<label className="field-label">Role</label>
			...
			<div className="delete-account-modal-overlay">
				<div className="delete-account-modal">
					<div className="delete-account-modal-header">
						<h3 className="delete-account-modal-title">
							حذف الحساب نهائياً
						</h3>
						<button
					...
					<div className="delete-account-modal-body">
						...
						<div className="delete-account-confirmation">
							<label htmlFor="delete-confirmation-input">
								للتأكيد، يرجى كتابة <strong>"حذف"</strong> في
								المربع أدناه:
							</label>
							<input
					...
					<div className="profile-header">
						<div className="profile-avatar-container">
							<div
								...
								style={{
									background: `linear-gradient(135deg, ${getRoleColor(role)}20, ${getRoleColor(role)}40)`,
								}}
							>
								<img
									src={
						...
						<div className="profile-actions-right">
							...
							>
								<span className="btn-icon">
									<i
										className={`fa-solid fa-pen-to-square medicina-theme-icon`}
									></i>
								</span>
							...
							>
								<span className="btn-icon">
									<i
										className={`fa-solid fa-key medicina-theme-icon`}
									></i>
								</span>
								Change Password
							</button>
							...
							{user.role === "doctor" && (
								<button
									onClick={() =>
										navigate(`/${user.role}/bio`)
									}
									className="modern-btn secondary-btn"
								>
									<span className="btn-icon">
										<i
											className={`fa-solid fa-notes-medical medicina-theme-icon`}
										></i>
									</span>
									Bio
								</button>
