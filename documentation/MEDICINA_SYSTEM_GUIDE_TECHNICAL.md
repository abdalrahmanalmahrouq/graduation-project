...
- **Key Fields**: full_name, specialty, phone_number, bio, consultation_duration
...
- **Route**: `/clinics`
- **API**: `GET /api/doctors/by-specialty/{specialty}`
- **Features**:
    - Filter by specialty (Pediatrics, Cardiology, etc.)
...
- **Returns**:
    - Doctor information (name, specialty, bio)
    - Associated clinics
...
| Method | Endpoint                                                         | Description              | Auth Required | Role   |
| ------ | ---------------------------------------------------------------- | ------------------------ | ------------- | ------ |
| GET    | `/api/doctors/by-specialty/{specialty}`                          | Get doctors by specialty | No            | -      |
...
- `/clinics/:directory` - Specialty directory
...
2. Clicks "العيادات" (Clinics)
   ↓
3. Selects specialty (e.g., "Pediatrics")
   ↓
